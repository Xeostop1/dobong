...
public class MemberDAO {
	...
	//연결처리 메서드 getConn
		...
		public int insertMember(MemberDTO m) {
			...
			String sql= "insert into register(id,nickname,name,password, phone, email, address, tos1, tos2, admin, meter)"
					+ "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			Connection conn=null;
...
//				1.해당 아이가 존재한다면
		...
		// 회원정보 수정
		//=====================
		public void updateRegister(MemberDTO DTO) {
			String sql="update register set nickname=?, name=?, password=?, phone=?, address=? where id=?";
			Connection conn=null;
			PreparedStatement pstmt=null;
			try {
				conn=getConn();
				...
				System.out.println("updateRegister() sql 접속오류"+e);
			}finally {
				MemberDAO.close(conn, pstmt);
			}
		}
